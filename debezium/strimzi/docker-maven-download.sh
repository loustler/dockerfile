...
## THIS FROM https://github.com/debezium/docker-images/blob/main/connect-base/1.9/docker-maven-download.sh
...
maven_debezium_plugin() {
    ...
    tar -xzf "$DOWNLOAD_FILE" -C "$KAFKA_PLUGIN_DESTINATION" && rm "$DOWNLOAD_FILE"
}
...
maven_apicurio_converter() {
    ...
    mkdir "$MAVEN_DEP_DESTINATION/apicurio"
    tar -xzf "$DOWNLOAD_FILE" -C "$MAVEN_DEP_DESTINATION/apicurio" && rm "$DOWNLOAD_FILE"
}
